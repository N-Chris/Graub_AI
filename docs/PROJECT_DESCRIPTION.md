...
3. **A measurable efficiency gain over a single-agent baseline** — `baseline_single_agent.py` runs the identical scenario through one generalist LLM call with the same constraints stated in its prompt; `run_comparison.py` runs the same scenarios through both systems and times them. Across 5 real scenarios: Finance and/or IT issued a genuine critique in 35 of 23 total subtasks' negotiation rounds (avg. 1.5 rounds per subtask, against a 2-round cap) — meaning independent review caught something worth revising in most proposals *before* a human ever saw them. That costs real time: multi-agent runs averaged ~131s against baseline's ~14s (about 9x), because it's doing genuinely more work — one unaudited generalist answer vs. multiple specialist drafts, two independent audits per draft, and revision cycles when something's wrong. The efficiency gain isn't speed; it's how much a human's own review burden shrinks when most problems are already caught and fixed by the time something reaches them. Results are rendered directly on the live dashboard (`comparison_results.json` in the repo has the raw numbers), not left in a file judges have to dig for.
